...
# کوکی اینستاگرام (اختیاری ولی برای دانلود ریلز تقریباً همیشه لازمه).
# محتوای کامل یه فایل cookies.txt (فرمت Netscape) از یه اکانت لاگین‌شده اینستا.
...
async def download_and_send(user_id: int, url: str, platform: str, height: int = None):
    ...
    if platform == "instagram" and INSTAGRAM_COOKIES_PATH:
    ...
    try:
        ...
        logger.error("Download failed: %s", err_text)
        await bot.send_message(
            user_id,
        ...
        if ADMIN_ID:
            short_err = html.escape(err_text[-3500:])  # تلگرام محدودیت طول پیام داره
            ...
            try:
                ...
                logger.error("Could not deliver error report to admin: %s", send_err)
        return
